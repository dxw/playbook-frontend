...
RSpec.describe 'Playbook Overlay Application' do
  ...
  describe 'Open Graph meta properties' do
    ...
    context 'when @title is the default playbook title' do
      ...
      it 'includes the full title even if it matches the default' do
        expect(last_response.body).to include("<meta property=\"og:title\" content=\"dxw's Playbook - Playbook - dxw\" />")
      end
    end
  end
